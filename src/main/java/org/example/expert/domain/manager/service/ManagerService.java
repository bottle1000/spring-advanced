...
public class ManagerService {
    ...
    public ManagerResponse saveManager(AuthUser authUser, long todoId, ManagerSaveRequest managerSaveRequest) {
        ...
        Todo todo = todoRepository.findById(todoId)
                .orElseThrow(() -> new NotFoundUserException("Todo not found"));

        /**
         * 이미 todo.getUser 자체가 null인데 null.getId()를 호출해서 NPE가 발생함.
         */
        if (!ObjectUtils.nullSafeEquals(user.getId(), todo.getUser())){
            throw new UnauthorizedTodoAccessException("담당자를 등록하려고 하는 유저가 일정을 만든 유저가 유효하지 않습니다.");
        }
        ...
        if (ObjectUtils.nullSafeEquals(user.getId(), managerUser.getId())) {
            throw new SelfAssignmentException("일정 작성자는 본인을 담당자로 등록할 수 없습니다.");
        }
    ...
    public List<ManagerResponse> getManagers(long todoId) {
        Todo todo = todoRepository.findById(todoId)
                .orElseThrow(() -> new NotFoundTodoException("Todo not found"));
    ...
    public void deleteManager(long userId, long todoId, long managerId) {
        ...
        if (todo.getUser() == null || !ObjectUtils.nullSafeEquals(user.getId(), todo.getUser().getId())) {
            throw new UnauthorizedTodoAccessException("해당 일정을 만든 유저가 유효하지 않습니다.");
        }

        Manager manager = managerRepository.findById(managerId)
                .orElseThrow(() -> new NotFoundUserException("Manager not found"));

        if (!ObjectUtils.nullSafeEquals(todo.getId(), manager.getTodo().getId())) {
            throw new InvalidManagerAssignmentException("해당 일정에 등록된 담당자가 아닙니다.");
        }
